...
# This file was *autogenerated* from the file coron.sage
from sage.all_cmdline import *   # import sage library

_sage_const_2 = Integer(2); _sage_const_0 = Integer(0); _sage_const_1 = Integer(1); _sage_const_3 = Integer(3); _sage_const_512 = Integer(512); _sage_const_300 = Integer(300); _sage_const_4 = Integer(4)
import itertools

def coppersmith_bivariate(p, X, Y, k = _sage_const_2 , i_0 = _sage_const_0 , j_0 = _sage_const_1 , debug=True):
    """
    ...
    """
    if len(p.variables()) != _sage_const_2 :
    ...
    if debug and X * Y > W**(_sage_const_2 /(_sage_const_3 *d)):
        print("Warning: XY > W^(2/(3*d)); a solution may not be found.")
    

    prods = list(itertools.product(range(k), range(k)))[::-_sage_const_1 ]
    prods_kd = list(itertools.product(range(k + d), range(k + d)))[::-_sage_const_1 ]
    ...
    f = sum(x**i for i in range(terms[_sage_const_0 ].degree() // _sage_const_2  + _sage_const_2 ))
    f *= f(x=y)
    
    highest_order = terms[_sage_const_0 ]
    ...
    rest = [t for t in list(zip(*list(f)))[_sage_const_1 ] if max(t.degree(x), t.degree(y)) <= d2 and t not in terms]
    s_terms = terms + rest    

    # Builds the matrix S and calculates n = |det S|.
    X_dim, Y_dim = k**_sage_const_2 , k**_sage_const_2 
    ...
    for r, (a, b) in enumerate(prods):
        ...
        row = vector([s_dict[t] if t in s_dict else _sage_const_0  for t in terms])
        S[r] = row
    ...
    # 0 <= a, b <= d.
    X_dim, Y_dim = k**_sage_const_2  + (k + d)**_sage_const_2 , (k + d)**_sage_const_2 
    ...
    for r, (a, b) in enumerate(prods):
        ...
        coeffs, mons = zip(*list(s_ab))
        s_dict = {k: v for k, v in zip(mons, coeffs)}
        row = vector([s_dict[t] * t(x=X, y=Y) if t in s_dict else _sage_const_0  for t in s_terms])
        M[r] = row
    ...
    for r, (i, j) in zip(range(k**_sage_const_2 , X_dim), prods_kd):
        r_ab = x**i * y**j * n
        coeffs, mons = zip(*list(r_ab))
        r_dict = {k: v for k, v in zip(mons, coeffs)}
        row = vector([n * t(x=X, y=Y) if t in r_dict else _sage_const_0  for t in s_terms])
        M[r] = row
    ...
    # Performs LLL on L_2
    L_2 = M[list(range(k**_sage_const_2 , k**_sage_const_2  + l)), list(range(k**_sage_const_2 , k**_sage_const_2  + l))].LLL()
    ...
    h = sum(coeff * term // (X**term.degree(x) * Y**term.degree(y)) for (coeff, term) in zip(L_2[_sage_const_0 ], rest))
    ...
    for x_0 in roots_x:
        ...
        roots_y.append(y_0[_sage_const_0 ] if y_0 else None)

    if debug and len(roots_x) > _sage_const_0  and len(roots_y) > _sage_const_0 :
...
if __name__ == "__main__":
    ...
    prime_size = _sage_const_512 
    p = random_prime(_sage_const_1 <<prime_size, proof=False)
    q = random_prime(_sage_const_1 <<prime_size, proof=False)

    N = p * q

    # Lower bits of p
    lower = _sage_const_300 
    bits = prime_size - lower
    mask = _sage_const_1  << bits
    ...
    X, Y = mask << _sage_const_1 , mask << _sage_const_1 
    ...
    print(f"N = {N}")
    print("\n")
    res = coppersmith_bivariate(poly, X, Y, k=_sage_const_4 )
    print("\n")
    if not res:
        raise ValueError("No roots found.")
    
    x_s, y_s = res
    p_r, q_r = _sage_const_0 , _sage_const_0 
